...
export class CheckoutComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.totalProductos = this.carrito.reduce(
  ...
  cargarRegiones() {
    ...
    this.http
      .get<any>('http://localhost:3000/java-api/region', { headers })
      .subscribe({
        next: (response) => {
          ...
          this.regiones = regionesCrudas.map((region: any) => ({
            ...
            ciudades:
              region.ciudades?.map((nombreCiudad: string, index: number) => ({
                id: index + 1,
                nombre: nombreCiudad,
              })) || [],
          }));
  ...
  onRegionChange() {
    const regionSeleccionada = this.regiones.find(
      (r) => r.regionId == this.datosDespacho.region
    );
  ...
  registrarInvitado() {
    ...
    const password = this.deseaRegistrarse
      ? clave
      : `inv_${rut.replace('-', '')}`;
    ...
    this.http
      .post<any>('http://localhost:3000/java-api/usuario', datos)
      .subscribe({
        next: (usuarioCreado) => {
  ...
  procesarPedido(clienteId: number) {
    ...
    this.http
      .post<any>('http://localhost:3000/java-api/pedido', pedido, { headers })
      .subscribe({
        next: (response) => {
          if (isPlatformBrowser(this.platformId)) {
